...
def parse_args():
    ...
    p.add_argument(
        "--target",
        required=True,
        help="Base URL target (contoh: https://example.com)"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Kedalaman crawl (default 1)"
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=2.0,
        help="Rate limit RPS (default 2)"
    )

    p.add_argument(
        "--out",
        default="report.json",
        help="Path file JSON report"
    )

    p.add_argument(
        "--html",
        default="report.html",
        help="Path file HTML report"
    )

    p.add_argument(
        "--scope",
        choices=["same-domain", "same-host"],
        default="same-domain",
    ...
    p.add_argument(
        "--pdf",
        default=None,
        help="Path file PDF report (opsional)"
    )

    p.add_argument(
        "--auth-bruteforce",
        action="store_true",
        help="Enable authentication bruteforce test"
    )

    p.add_argument(
        "--ai-analysis",
        action="store_true",
        help="Enable AI analysis with DeepSeek"
    )
...
def main():
 ...
 |_____/_____|_____/|______|_|      |_|  |______|_____/   |_|     (_) |
    ...
    if args.ai_analysis:
        print("🤖 AI Analysis: Enabled (DeepSeek V4 Flash Free)")
    ...
    if args.ai_analysis:
        ...
        ai_loader = SimpleLoader("🤖 Running AI analysis with DeepSeek")
        ai_loader.start()
        ...
        try:
            ...
            # Preserve original DAST findings
